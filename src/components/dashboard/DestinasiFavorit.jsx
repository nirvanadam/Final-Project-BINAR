...
function DestinasiFavorit() {
  ...
  const DestinasiCard = () => {
    return data?.map((destinasi, index) => {
      return (
        <div key={index} className="bg-zinc-100 relative inline-flex flex-col lg:mt-3 mr-5 lg:mr-0 overflow-hidden group rounded-xl shadow-md cursor-default transition-all duration-[0.4s] hover:-translate-y-3 whitespace-normal	">
          <div className="overflow-hidden relative h-[200px] before:content-[''] before:absolute before:top-0 before:bottom-0 before:left-0 before:right-0 before:bg-gradient-to-t from-black from-5% before:z-10">
            <img key={index} src={`https://source.unsplash.com/1600x900/?kota&indnesia&${destinasi.data.arrival}`} alt="" className="absolute transition-all duration-[0.4s] group-hover:scale-125" />
          ...
          <div className="flex flex-col items-center gap-4 p-6">
            <h1 className="text-xl font-semibold">
              {formatCurrency(destinasi.data.price)}
              <span className="text-base text-gray-500 font-medium">/ Orang</span>
            </h1>
            ...
            <button onClick={() => handleOnClick(destinasi.id)} className="bg-white border border-gray-300 rounded-full px-10 py-3 text-sm font-semibold tracking-[2px] duration-300 transition-all hover:bg-primary hover:text-white">
              ORDER
            </button>
          </div>
        </div>
      ...
      <section>
        <div className="relative overflow-hidden bg-[url('/images/edinburgh-poster.jpg')] bg-cover bg-fixed bg-center flex justify-center items-center h-[300px] lg:h-[600px] border border-red-50 before:content-[''] before:absolute before:bg-black before:top-0 before:bottom-0 before:left-0 before:right-0 before:opacity-40">
          <div className="flex flex-col items-center z-10">
            <h1 className="text-lg lg:text-2xl text-white font-bold italic">Get Your Ticket and Flight With Us</h1>
            <h1 className="text-4xl lg:text-9xl text-white font-bold">Destination</h1>
          </div>
        </div>
